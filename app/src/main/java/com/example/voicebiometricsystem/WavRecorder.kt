...
class WavRecorder() {
    ...
    private var maxAmplitude: Float? = null

    fun startRecording(filename: String? = null, dirPath: String? = null) {
    ...
    private fun short2byte(sData: ShortArray): ByteArray {
        ...
        // Calculate amplitude and notify the waveform view
        maxAmplitude = calculateAmplitude(sData)
    ...
    private fun calculateAmplitude(sData: ShortArray): Float {
        // Calculate the amplitude of the audio data
        var sum = 0.0
        for (sample in sData) {
            sum += sample * sample
        }
        val rms = sqrt(sum / sData.size)
        return rms.toFloat()
    }
